...
export class RealtimeNodejs {
  ...
  constructor(private context: EndpointContext) {
    ...
    if (!context.disabledRealtime) {
      ...
      ioGlobalNsp.on('connection', clientSocket => {
        log.i(
          `client conected to namespace "${clientSocket.nsp.name}",  host: ${context.host}`,
        );
      });

      log.i(
        `CREATE GLOBAL NAMESPACE: '${ioGlobalNsp.path()}' , path: '${nspPath.global.pathname}'`,
      );
      ...
      log.i(
        `CREATE REALTIME NAMESPACE: '${ioRealtimeNsp.path()}' , path: '${nspPath.realtime.pathname}' `,
      );
      ...
      ioRealtimeNsp.on('connection', backendSocketForClient => {
        log.i(
          `client conected to namespace "${backendSocketForClient.nsp.name}",  host: ${context.host}`,
        );
  ...
  private static __TrigggerEntityChanges(
    ...
    if (customEvent) {
      ...
      if (!valueOfUniquPropery) {
        Helpers.error(
          `[Firedev][Realtime] Entity without iD ! ${ClassHelpers.getName(entityFn)} `,
          true,
          true,
        );
        return;
    ...
    const job = () => {
      ...
      base.BE_REALTIME.in(roomName).emit(
  ...
  public static TrigggerEntityChanges(
    entityObjOrClass: Function,
  ...
  public static TrigggerEntityPropertyChanges<ENTITY = any>(
    ...
    if (context.disabledRealtime) {
      ...
      console.warn(
        `[Firedev][TrigggerEntityPropertyChanges][property=${property as string}] Entity "${className}' is not realtime`,
      );
      return;
    }
  ...
  public static TrigggerCustomEvent(
    context: EndpointContext,
    customEvent: string,
    dataToPush: any,
  ) {
