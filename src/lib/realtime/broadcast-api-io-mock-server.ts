...
const log = Log.create('[SERVER] broadcast api mock', Level.__NOTHING);

//#region server - namespace
export class BroadcastApiIoMockServerSocket {
  constructor(public readonly server: BroadcastApiIoMockServer) {}

  private get currentClient() {
    const ins = BroadcastApiIoMockClient._isntanceBy(this.server._url.origin, {
  ...
  leave(roomName: string) {
    const roomToClose = IsomorphicBroadCastChannel.for(
      roomName,
      this.server._url.href,
    );
    if (roomToClose) {
      roomToClose.close();
      this.currentClient.allowedToListenRooms =
        this.currentClient.allowedToListenRooms.filter(
          allowedRoomTolisten => allowedRoomTolisten !== roomName,
        );
    }
  }
}
...
export class BroadcastApiIoMockServer {
  ...
  on(
    ...
    callback: (socket: BroadcastApiIoMockServerSocket) => any,
  ) {
    setTimeout(() => {
