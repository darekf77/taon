...
to use socket realtime connection;
      ...
      if (customEvent) {
        ...
        roomName = _.isString(property) ?
          Symbols.old.REALTIME.ROOM_NAME.UPDATE_ENTITY_PROPERTY(className, property, idOrUniqValue) :
          Symbols.old.REALTIME.ROOM_NAME.UPDATE_ENTITY(className, idOrUniqValue);
      }
  ...
  */
  static listenChangesEntityObj(entity, options?: RealtimeBrowserRxjsOptions) {
    ...
    return RealtimeBrowserRxjs.listenChangesEntity(classFn, entity[uniqueKey], options);
  }
  //#endregion

  //#region listen changes custom event
  static listenChangesCustomEvent(context: EndpointContext, customEvent: string) {
