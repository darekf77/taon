...
export namespace ClassHelpers {
  ...
  export const getName = (classFnOrObject: any): string => {
    ...
    return (classFnOrObject[Symbols.classNameStaticProperty]
      ...
      : classFnOrObject?.constructor[Symbols.classNameStaticProperty]) || void 0;
  };
  //#endregion

  //#region get uniqu key
  export const getUniquKey = (classFnOrObject: any): string => {
    const classFn = _.isFunction(classFnOrObject) ? classFnOrObject : classFnOrObject.constructor;
    const config = Reflect.getMetadata(Symbols.metadata.options.controller, classFn) as FiredevEntityOptions;
  ...
  export const hasParentClassWithName = (target: Function, className: string, targets = []): boolean => {
    if (!target) {
      return false;
    }
    targets.push(target)
    ...
    if (_.isFunction(targetProto) && ClassHelpers.getName(targetProto) === className) {
  ...
  export const getControllerConfig = (target: Function): Models.ControllerConfig | undefined => {
    const classMetadataOptions: FiredevControllerOptions = Reflect.getMetadata(Symbols.metadata.options.controller, target);
    const classMetadata: Models.ControllerConfig = _.merge(new Models.ControllerConfig(), classMetadataOptions);
    ...
    for (const methodName of methodNames) {
      const methodMetadata: Models.MethodConfig = Reflect
        .getMetadata(Symbols.metadata.options.controllerMethod, target, methodName);
  ...
  export const getMethodsNames = (classOrClassInstance: any, allMethodsNames = []): string[] => {
  ...
  export const getControllerConfigs = (target: Function, configs: Models.ControllerConfig[] = [], callerTarget?: Function): Models.RuntimeControllerConfig[] => {
    ...
    return isValidParent ? getControllerConfigs(parentClass, configs, target) : configs;
  }
  //#endregion
