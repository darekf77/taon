...
export class EndpointContext {
  ...
  private getAutoGeneratedConfig(): Models.DatabaseConfigTypeOrm {
    ...
    if (Helpers.isRunningInDocker()) {
      ...
      databaseConfig = Models.DatabaseConfig.from({
        database: `${_.kebabCase(this.contextName)}.sqlite`,
        type: 'mysql',
      ...
      switch (this.mode) {
        ...
        case 'backend-frontend(ipc-electron)':
          databaseConfig = Models.DatabaseConfig.from({
            location: `db-${_.kebabCase(this.contextName)}.sqlite`,
            type: 'sqljs',
            // autoSave: true,
  ...
  async initDatabaseConnection() {
    ...
    const migrations = this.config.override?.migrations
      ? this.config.override.migrations
      : this.getClassFunByArr(Models.ClassType.MIGRATION);
    ...
    const dataSourceDbConfig = _.isObject(this.databaseConfig)
      ? ({
          ...
          migrations,
          migrationsTableName: 'taon_migration_table',
          entities,
          subscribers,
