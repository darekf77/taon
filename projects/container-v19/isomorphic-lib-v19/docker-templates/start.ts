...
console.clear();
const COMPOSE_PROJECT_NAME = process.env.COMPOSE_PROJECT_NAME || 'container-v19__isomorphic-lib-v19';
const env = { ...process.env, COMPOSE_PROJECT_NAME };

const child = spawn('docker-compose', ['-f',  'docker-compose.yml', 'up', '--build'], {
  env,
  cwd: __dirname,
...
process.stdin.on('data', () => {
  child.kill('SIGINT');
  console.log('Exiting...');
  const downProcess = spawn('docker-compose', ['-f', 'docker-compose.yml', 'down'], {
    env,
    cwd: __dirname,
    stdio: 'inherit',
  });
